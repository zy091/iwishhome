                ...
                <el-card v-for="test in tests" :key="test.id" class="test-card" shadow="hover">
                    <div class="test-card-content">
                        <div class="test-type-tag flex ">
                            ...
                            <el-tag :type="getTagType(test.chapter || '')">{{ getTestTypeLabel(test.chapter || '')
                                }}</el-tag>
                        </div>
...
import PersonalTestHistory from './PersonalTestHistory.vue'
...
const loadTests = async () => {
    ...
    try {
        const { data, total } = await testService.getTests({
            ...
            chapter: chapter.value as string
        })
    ...
    if (getTestResult.value(test.id)?.completion_status === 'completed') {
        ...
        ElMessageBox.confirm('您已完成此测验，确定要重新开始吗？', '提示', {
            ...
            router.push({
                ...
                query: {
                    testId: test.id,
                    testName: getTestTypeLabel(test.platform || '') + '-' + getTestTypeLabel(test.chapter || '') + '-' + test.title,
